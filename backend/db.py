...
def init_db():
    ...
    # First run: create a single Super Admin account.
    #
    # If SUPERADMIN_USERNAME / SUPERADMIN_PASSWORD are set as environment
    # variables (e.g. in Render → Environment), those fixed credentials are
    # used every time the account has to be (re)created. This matters on
    # platforms without persistent disk: the SQLite file can be wiped on every
    # restart/redeploy, but env vars are NOT wiped, so the same login keeps
    # working instead of a new random password being generated each time.
    #
    # If no env vars are set, we fall back to the old behaviour: a random
    # password generated once and printed to the console.
    env_user = os.environ.get("SUPERADMIN_USERNAME", "superadmin").strip() or "superadmin"
    env_pass = os.environ.get("SUPERADMIN_PASSWORD")

    if not get_user(env_user):
        if env_pass:
            create_user(env_user, env_pass, role="superadmin", full_name="Super Admin")
            print(
                f"\n[SENTINEL OSINT] Superadmin '{env_user}' created from "
                f"SUPERADMIN_USERNAME/SUPERADMIN_PASSWORD env vars.\n",
                flush=True,
            )
        else:
            pw = secrets.token_urlsafe(12)
            create_user(env_user, pw, role="superadmin", full_name="Super Admin")
            banner = (
                "\n" + "=" * 62 +
                ...
                "\n  ▸ Set SUPERADMIN_PASSWORD as an env var to make this survive restarts." +
                "\n" + "=" * 62 + "\n"
            )
